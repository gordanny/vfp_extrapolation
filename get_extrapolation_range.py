def get_extrapolation_range(data, parameters):
    ...
    for parameter in parameters_dict:
        if parameters_dict[parameter] < data['parameters'][parameter]['values'][0]:
        ...
        if parameters_dict[parameter] > data['parameters'][parameter]['values'][-1]:
            parameters_range[parameter] = {
                'type': 'single',
                'min': data['parameters'][parameter]['values'][-2],
                'max': data['parameters'][parameter]['values'][-1]
            }
            continue

        for value in data['parameters'][parameter]['values']:
            ...
            if parameters_dict[parameter] < value:
                ...
                if value_index == len(data['parameters'][parameter]['values']) - 2:
                    parameters_range[parameter] = {
                        'type': 'single',
                        'min': data['parameters'][parameter]['values'][-3],
                        'max': data['parameters'][parameter]['values'][-2]
                    }
                    break
                parameters_range[parameter] = {
                    'type': 'double',
                    'min': data['parameters'][parameter]['values'][value_index],
                    'low_mid': data['parameters'][parameter]['values'][value_index-1],
                    'high_mid': data['parameters'][parameter]['values'][value_index],
                    'max': data['parameters'][parameter]['values'][value_index-1]
                }
                break
